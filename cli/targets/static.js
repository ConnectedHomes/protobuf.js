...
function static_target(root, options, callback) {
    ...
    try {
        ...
        /* Set Properties */
        push("");
        push("function setProperties (context, properties) {");
        indent++;
        push("return properties && Object.keys(properties).forEach(function(k) {");
        indent++;
        push("if(properties[k] != null) {");
        indent++;
        push("context[k] = properties[k];");
        indent--;
        push("}");
        indent--;
        push("});");
...
function aOrAn(name) {
    return (
        ((/^[hH](?:ou|on|ei)/.test(name) || /^[aeiouAEIOU][a-z]/.test(name)) && !/^us/i.test(name) ? "an " : "a ") +
        name
    );
}
...
function buildType(ref, type) {
    ...
    /* Constructor */
    push("");
    push("function " + escapeName(type.name) + "(properties) {");
    indent++;
    push("setProperties(this, properties);");
    indent--;
    push("}");
    /* Constructor */
    ...
    //field by number
    push("");
    push((config.es6 ? "const" : "var") + " fieldNameMap = {");
    ++indent;
    type.fieldsArray.forEach(function(field, i) {
        field.resolve();
        push(field.id + ": '" + field.name + "'" + (i < type.fieldsArray.length - 1 ? "," : ""));
    });
    --indent;
    push("};");
    ...
    ++indent;
    push((config.es6 ? "const" : "var") + " num = Object.keys(fieldNameMap).find(key => fieldNameMap[key] === name);");
    ...
    ++indent;
    push("return fieldNameMap[num];");
    --indent;
    push("};");

    push("");
    push(escapeName(type.name) + ".prototype.fieldByNumber = " + escapeName(type.name) + ".fieldByNumber;");
    ...
    if (config.convert) {
        ...
        push(escapeName(type.name) + ".prototype.toObject = function toObject() {");
        ++indent;
